export let clearErrors = function (form) {
    let errors = form.querySelectorAll('.error'); // возвращение всех найденных элементов 
...
export let redirect = function(reference) {
    location.href = reference; // юрл текущей страници, когда вызовется функция 
}

export let checkName = function (firstname, errors) {
    let nameErrors = []; // вункция проверить имя передается значение имени и ошибки, 
...
export let checklast_name = function (lastname, errors) {
    let lastnameErrors = [];// вункция проверить фамилии передается значение фамилии и ошибки, 
...
export let changeColor = function (element, elementErrors, errors) {
    ...
    } // работа с цветами ошибок 

    return errors;
}

export let checkEmail = function (email, errors) {
    let emailErrors = []; // вункция проверить маил передается значение маил и ошибки, 
...
export let formEvent = function (form, errorsBlock, urlRequest, urlRedirect) {
    let captcha = grecaptcha.getResponse(); // доткрывает пост, пишет что все ок, 
    ...
    if (errorsBlock.innerHTML == ''){
        ...
        request.onload = () => {
            if (request.status !== 200) {
                return; //н ок, закрывается 
            }

            let response = request.response;

            if (response.status == 'ERROR') {
                grecaptcha.reset(); // ошибка, ресет капча 
            ...
            else
            {
                ...
                setTimeout(redirect, 2000, urlRedirect); // все ок направляет на юрлредиктет
            }
...
export let check_patronymic_name = function (patronymicname, errors) {
    let patronymicnameErrors = []; // вункция проверить отчество передается значение отчесва и ошибки, 

        let regExp = /^[А-Яа-яЁё" .(),-]+$/g; //регулярка для отчества 
